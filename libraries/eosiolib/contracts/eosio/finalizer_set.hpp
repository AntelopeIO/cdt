...
namespace eosio {
    ...
    struct abi_finalizer_authority {
        ...
        std::vector<char>     public_key_g1_jacobian; // Jacobian little endian

        EOSLIB_SERIALIZE(abi_finalizer_authority, (description)(fweight)(public_key_g1_jacobian));
    };
    ...
    void set_finalizers( const abi_finalizer_set& fin_set ) {
        for (const auto& finalizer : fin_set.finalizers)
            eosio::check(finalizer.public_key_g1_jacobian.size() == sizeof(bls_g1), "public key has a wrong size" );
